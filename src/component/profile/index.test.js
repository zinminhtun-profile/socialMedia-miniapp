  ...
  test("render profile component", () => {
    ...
    render(
      ...
      <Router history={history}>
          <Profile 
          name="meww"
          profile={`https://instagram.frgn2-2.fna.fbcdn.net/v/t51.2885-19/s150x150/11856603_940548069324829_395577565_a.jpg?_nc_ht=instagram.frgn2-2.fna.fbcdn.net&_nc_ohc=x9xpMGBtW6kAX9zSbLt&tp=1&oh=11d9e13bcde344737f46f67d10ad4202&oe=601736B6`}
    ...
    const linkElement = screen.getByTestId("profile-avatar");
    ...
    const externalUrlElement = screen.getByTestId("external_url");
    expect(externalUrlElement).toBeInTheDocument();

    const followCountElement = screen.getByTestId("follow_count");
    expect(followCountElement).toBeInTheDocument();

    const postCountElement = screen.getByTestId("post_count");
    expect(postCountElement).toBeInTheDocument();

    const followingCountElement = screen.getByTestId("following_count");
    expect(followingCountElement).toBeInTheDocument();

    const locationListElement = screen.getAllByTestId("location");
    expect(locationListElement.length).toEqual(2)
